...
# %%
# def load_RL_model_if_exists(model, samples, generation, save_path):
#     model_filename = f"{save_path}dynamic_RL_model_N{N}_s{samples}_g{generation}.pth"
...
def save_RL_model(model, samples, generation, save_path, sigma, alpha):
    os.makedirs(save_path, exist_ok=True)
    model_filename = f"{save_path}dynamic_RL_model_N{N}_s{samples}_g{generation}_sig{sigma}_a{alpha}.pth"
    torch.save(model.state_dict(), model_filename)
    print(f"Model saved at generation {generation} to {model_filename}", flush=True)
    logging.info(f"Model saved at generation {generation} to {model_filename}")

def train_ES(model, training_data, num_generations=100, sigma=0.1, alpha=0.01, population_size=10, save_interval=10, save_path='models/es/'):
    """
    Evolution Strategies (ES) Training Loop
    :param model: Neural network model
    :param training_data: List of agents
    :param num_generations: Number of iterations
    :param sigma: Standard deviation for perturbations
    :param alpha: Learning rate for updating parameters
    :param population_size: Number of sampled perturbations per generation
    :param save_interval: Number of generations before saving the model
    :param save_path: Directory to save the trained model
    """

    for generation in range(num_generations, 0, -save_interval):
      model_filename = f"{save_path}dynamic_RL_model_N{N}_s{len(training_data)}_g{generation}_sig{sigma}_a{alpha}.pth"
      if os.path.exists(model_filename):
          model.load_state_dict(torch.load(model_filename))
          print(f"Resuming training from generation {generation}", flush=True)
          logging.info(f"Resuming training from generation {generation}")
          start_generation = generation
          break
      else:
          start_generation = 0  # No saved models found, start from scratch

    params = {name: param for name, param in model.named_parameters()}
    print(f"Training ES for {num_generations} generations with population size {population_size}", flush=True)

    for generation in tqdm(range(start_generation, num_generations)):
        perturbations = {}  # Stores noise samples for each parameter
        rewards = np.zeros(population_size)

        # Generate perturbations for each individual in the population
        for i in range(population_size):
            noise = {name: torch.randn_like(param) * sigma for name, param in params.items()}
            perturbations[i] = noise
            ...
            for agents in training_data:
                input_data = preprocess_agents(agents).unsqueeze(0)  # Preprocess input
                predicted_probabilities = model(input_data)
                sampled_actions = sample_actions(predicted_probabilities)
                boolean_tree = boolean_list_to_tree(sampled_actions.tolist()[0], len(agents))
                total_reward += analyzeTree(boolean_tree, agents)

            rewards[i] = total_reward / len(training_data)  # Average reward

            # Remove perturbation
            for name, param in params.items():
                param.data -= noise[name]

        # Compute weighted sum of perturbations using rewards
        mean_reward = np.mean(rewards)
        std_reward = np.std(rewards) + 1e-8  # Prevent division by zero

        # Compute parameter update
        for name, param in params.items():
            update = sum((rewards[i] - mean_reward) * perturbations[i][name] for i in range(population_size)) / (population_size * std_reward)
            param.data += alpha * update  # Apply update

        print(f"Generation {generation + 1}: Average Reward = {mean_reward:.4f}, Sigma: {sigma}, Alpha: {alpha}", flush=True)
        logging.info(f"Generation {generation + 1}: Average Reward = {mean_reward:.4f}")

        # Save model every `save_interval` generations
        if (generation + 1) % save_interval == 0:
            save_RL_model(model, len(training_data), generation + 1, save_path, sigma, alpha)

    save_RL_model(model, len(training_data), num_generations, save_path, sigma, alpha)
...
training_data = getAgentsFromTrainData('data/training/training_data_N5_B3_G5.csv')
eval_data = getAgentsFromTrainData(f'data/training/eval_data_N{N}_B{B}_G{G}.csv')
...
def evaluate_model(model, training_data):
    ...
    with torch.no_grad():
        for agents in training_data:
            ...
            # )
            boolean_tree = boolean_list_to_tree(
                torch.bernoulli(predicted_probabilities).flatten().tolist(), len(agents)
            )
...
def train_and_evaluate(params, num_epochs, training_data, eval_data):
    ...
    model = UtilityMaximizingRLModel(input_size=input_size, output_size=output_size)
    
    # Train the model def train_ES(model, training_data, num_generations=100, sigma=0.1, alpha=0.01, population_size=10, save_interval=10, save_path='models_es/'):
    train_ES(model, training_data, num_generations=num_epochs, alpha=alpha, sigma=sigma)
    
    # Evaluate the model
    avg_reward = evaluate_model(model, eval_data)
    
    # Return the result
    return {"alpha": alpha, "num_epochs": num_epochs, "sigma": sigma, "avg_reward": avg_reward}
