...
# %%
def load_RL_model_if_exists(model, samples, epoch, beta, beta_decay, lr, save_path='new_models_G_Aug/'):
    model_filename = f"{save_path}dynamic_RL_model_N{N}_Seq_G{G}_B{B}_s{samples}_e{epoch}_b{beta}_lr{lr}_bd{beta_decay}_Aug.pth"
...
# Training loop using REINFORCE with model saving
def train_RL_model(model, optimizer, training_data, num_epochs, lr, beta, beta_decay, save_interval=100, save_path='new_models_G_Aug/'):
    ...
    for epoch in tqdm(range(start_epoch, num_epochs)):
        ...
        for agents in training_data:
            ...
            # now = time.time()
            reward = analyzeTree(boolean_tree, agents) + 10
        ...
        if (epoch + 1) % save_interval == 0:
            ...
            model_filename = f"{save_path}dynamic_RL_model_N{N}_Seq_G{G}_B{B}_s{len(training_data)}_e{epoch + 1}_b{beta}_lr{lr}_bd{beta_decay}_Aug.pth"
            torch.save(model.state_dict(), model_filename)
...
training_data = getAgentsFromTrainData(f'training_data_N{N}_Seq_G{G}_B{B}_G{G}.csv')[:num_samples]
eval_data = getAgentsFromTrainData(f'eval_data_N{N}_Seq_G{G}_B{B}_G{G}.csv')[:num_samples]
