...
# %%
def load_RL_model_if_exists(model, samples, epoch, beta, beta_decay, lr, save_path='new_models/'):
...
def preprocess_agents(agents, classOrder):
    ...
    counts = [sum(a < agent[2] <= b for agent in agents) for a, b in classOrder]
...
def compute_reward(predicted_ids, agents):
    ...
    agents = [(i, tup[1], tup[2]) for i, tup in enumerate(sorted(agents, key=lambda x: (x[1], x[2])))]
...
# Training loop using REINFORCE with model saving
def train_RL_model(model, optimizer, training_data, num_epochs, lr, beta, beta_decay, save_interval=5, save_path='new_models/'):
    if trial:
    ...
    for epoch in tqdm(range(start_epoch, num_epochs)):
        ...
        for index, agents in enumerate(training_data):
            # now = time.time()
            # print(f"on agent {index} in {now - epoch_time} seconds since epoch start, {now - agent_time} seconds since last agent", flush=True) 
            # agent_time = time.time()
            # Preprocess agents
            # print("begin epoch here", flush=True)
            input_data = preprocess_agents(agents, classOrder)
            input_data = input_data.unsqueeze(0)  # Add batch dimension

            # Forward pass
            # print("enter model", flush=True)
            logits = model(input_data)
            # print("exit model", flush=True)

            # Get discrete agent IDs
            predicted_ids = get_agent_ids(logits)
            # print("ids obtained", flush=True)

            # Compute reward based on predicted IDs
            reward = compute_reward(predicted_ids, agents)
            # print("reward obtained", flush=True)

            # Compute probabilities for entropy
            probabilities = F.sigmoid(logits)
            entropy = -torch.sum(probabilities * torch.log(probabilities))  # Avoid log(0)
            # print("entropy obtained", flush=True)
            ...
            loss = loss.mean()
            # print("loss obtained", flush=True)
            ...
            epoch_reward += reward
            # print("prop complete", flush=True)
            
        now = time.time()
        # print(f"completed epoch in {now - epoch_time} total seconds", flush=True)
...
# %%
# Load training data
training_data = getAgentsFromTrainData(f'training_data_N{N}_B{B}_G{G}.csv')[:num_samples]
eval_data = getAgentsFromTrainData(f'eval_data_N{N}_B{B}_G{G}.csv')[:num_samples]
print(f"Loaded from training_data_N{N}_B{B}_G{G}.csv for len {len(training_data)} and eval_data_N{N}_B{B}_G{G}.csv for len {len(eval_data)}")
logging.info(f"Loaded from training_data_N{N}_B{B}_G{G}.csv for len {len(training_data)} and eval_data_N{N}_B{B}_G{G}.csv for len {len(eval_data)}")
