...
# %%
def load_RL_model_if_exists(model, samples, epoch, beta, beta_decay, lr, save_path='N50_Seq_Bucket_Util1_Final/'):
...
# Training loop using REINFORCE with model saving
def train_RL_model(model, optimizer, training_data, num_epochs, lr, beta, beta_decay, save_interval=100, save_path='N50_Seq_Bucket_Util1_Final/'):
    ...
    for epoch in tqdm(range(start_epoch, num_epochs)):
        ...
        for agents in training_data:
            ...
            selection = {sorted_agents[x][0] for x in choices if 0 <= x < len(sorted_agents)}
