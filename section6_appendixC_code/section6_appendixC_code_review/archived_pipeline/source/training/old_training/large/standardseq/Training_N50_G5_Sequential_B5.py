...
B = 5  # Number of tests (defines output boolean list size)
G = 5
d = 3
input_size = N + 1 # Two relevant attributes per agent
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f"training_N{N}_Seq_G{G}_B{B}.log"),
...
# %%
def load_RL_model_if_exists(model, samples, epoch, beta, beta_decay, lr, save_path='N50_Seq_Fixed/', used_B=B):
    model_filename = f"{save_path}dynamic_RL_model_N{N}_Seq_G{G}_B{used_B}_s{samples}_e{epoch}_b{beta}_lr{lr}_bd{beta_decay}.pth"
...
def sample_actions(probabilities):
    # Get indices where probabilities are > 0.5
    valid_indices = torch.nonzero(probabilities > 0.5, as_tuple=True)[0]

    # If there are fewer than G valid indices, select all of them
    if len(valid_indices) <= G:
        selected_indices = valid_indices
    else:
        # Randomly select G indices from the valid indices
        selected_indices = valid_indices[torch.randperm(len(valid_indices))[:G]]
...
def preprocess_agents(agents):
    processed_agents = [agent[2] for agent in agents]  # Ignore the first attribute
    return torch.tensor(processed_agents, dtype=torch.float32).flatten()  # Flatten for input

################

def use_RL_model(model, agents, newB):
    """
    ...
    B_tensor = torch.tensor([newB], dtype=torch.float32, device=input_data.device).unsqueeze(0)  # Shape: (1, 1)
...
modelB2 = SeqRLModel(input_size=input_size, output_size=output_size)
load_RL_model_if_exists(modelB2, num_samples, 10, 0, 0, 0.01, used_B=2) # TODO
modelB3 = SeqRLModel(input_size=input_size, output_size=output_size)
load_RL_model_if_exists(modelB3, num_samples, 10, 1, 0.1, 0.01, used_B=3) # TODO
modelB4 = SeqRLModel(input_size=input_size, output_size=output_size)
load_RL_model_if_exists(modelB4, num_samples, 500, 1, 0, 0.001, used_B=4) # TODO
...
def nextAction(agents, newB):
   if newB >= 2:
      ...
      if newB == 4:
        m = use_RL_model(modelB4, agents, newB)
      ...
      if pNegative > 0:
        ...
        negAction = nextAction(negOutcomeAgents, newB-1)
          ...
          negAction = [False] * (N * (2**(newB-1) - 1)) 

      return m + posAction + negAction
...
# Training loop using REINFORCE with model saving
def train_RL_model(model, optimizer, training_data, num_epochs, lr, beta, beta_decay, save_interval=100, save_path='N50_Seq_Fixed/'):
    ...
    for epoch in tqdm(range(start_epoch, num_epochs)):
        ...
        for agents in training_data:
            ...
            input_data = torch.cat((B_tensor, input_data), dim=-1) 
            # print(f"Input data shape: {input_data.shape}", flush=True)
            # print(f"{time.time()-now}, for input", flush=True)
            # now = time.time()
            # Forward pass: predict probabilities for the boolean list
            predicted_probabilities = model(input_data)
            predicted_probabilities = torch.clamp(predicted_probabilities, 0.0, 1.0)
            ...
            # Sample actions based on the predicted probabilities
            # print(f"{time.time()-now}, to get output", flush=True)
            # now = time.time()
            ...
            # print(f"{time.time()-now}, to get sample", flush=True)
            # now = time.time()
            if pNegative < 1:
              ########## todo for larger B, not use bayes, use gibbs mcmc for both. apply changes to eval
              # print(f"{time.time()-now}, bayes", flush=True)
              ...
              posDict = bayesTheorem(agents, posGroups={selection}, negAgents={})
              posOutcomeAgents = sorted(((idNum, utility, health) for idNum, (utility, health) in posDict.items()), key=lambda x: x[0])
              # print(f"{time.time()-now}, pneg", flush=True)
              # now = time.time()
              posAction = nextAction(posOutcomeAgents, B-1)
              # print(f"{time.time()-now}, negAction", flush=True)
            else:
               posAction = [False] * (N * (2**(B-1) - 1)) 
            ...
            if pNegative > 0:
              ...
              negOutcomeAgents = sorted(((idNum, utility, health) for idNum, (utility, health) in negDict.items()), key=lambda x: x[0])
              # process here, if tested and negative
              # print(f"{time.time()-now}, pos", flush=True)
              # now = time.time()
              negAction = nextAction(negOutcomeAgents, B-1)
              # print(f"{time.time()-now}, actualAction", flush=True)
            else:
               negAction = [False] * (N * (2**(B-1) - 1)) 

            # print(f"{time.time()-now}, tree", flush=True)
            # now = time.time()
            #print(m.tolist(), flush=True)
            #print(posAction, flush=True)
            #print(negAction, flush=True)
            ...
            # directtime.append(time.time()-now)
            # now = time.time()
            # reward = analyzeTreeGibbs(boolean_tree, agents)[0]
            # gibbstime.append(time.time()-now)
            # if len(directtime) >= 50 and len(gibbstime) >= 50:
            #    print(sum(directtime)/len(directtime), flush=True)
            #    print(sum(gibbstime)/len(gibbstime), flush=True)
            #    print(xyz)
            # print(f"Boolean tree: {boolean_tree}")
            # print(f"{time.time()-now}, decipher reward", flush=True)
            # now = time.time()
...
num_epochs = [5, 10, 15, 25, 100, 250, 500]
num_epochs.sort()
