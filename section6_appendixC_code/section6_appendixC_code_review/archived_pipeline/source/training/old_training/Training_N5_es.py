...
# Configure logging with auto-flushing
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(processName)s - %(message)s",
    handlers=[
        logging.FileHandler(f"training_N{N}_Es_G{G}_B{B}.log"),
        FlushStreamHandler(stream=sys.stdout)  # Use custom handler with auto-flush
    ]
)

# %%
def load_RL_model_if_exists(model, samples, epoch, beta, beta_decay, lr, save_path='models_es/'):
...
def save_RL_model(model, samples, generation, save_path):
...
def train_ES(model, training_data, num_generations=100, sigma=0.1, alpha=0.01, population_size=10, save_interval=10, save_path='models_es/'):
    """
    ...
    for generation in tqdm(range(num_generations)):
        ...
        if (generation + 1) % save_interval == 0:
            save_RL_model(model, len(training_data), generation + 1, save_path)

    save_RL_model(model, len(training_data), num_generations, save_path)
...
def train_and_evaluate(params, training_data, eval_data):
    ...
    train_ES(model, training_data, num_generations=num_epochs, alpha=lr)
