...
# %%
def load_RL_model_if_exists(model, samples, epoch, beta, beta_decay, lr, save_path='models_edits/'):
...
class UtilityMaximizingRLModel(nn.Module):
    ...
    def _initialize_weights(self):
        ...
        # Set high positive bias for final layer to ensure sigmoid outputs ~1
        self.fc3.bias.data.fill_(5.0)  # Adjust the value if necessary
...
# Training loop using REINFORCE with model saving
def train_RL_model(model, optimizer, training_data, num_epochs, lr, beta, beta_decay, save_interval=100, save_path='models_edits/'):
    ...
    for epoch in tqdm(range(start_epoch, num_epochs)):
        ...
        for agents in training_data:
            ...
            for i in range(0, len(boolean_tree), N):
              if all(not boolean_tree[j] for j in range(i, min(i+N, len(boolean_tree)))):
                 reward += -N
