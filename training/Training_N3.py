...
#     handlers=[
#         logging.FileHandler(f"training_N{N}_G{G}_B{B}_{name}.log"),
...
# %%
def load_RL_model_if_exists(model, samples, epoch, beta, beta_decay, lr, save_path=f'N{N}_B{B}_G{G}_{name}/'):
    model_filename = f"{save_path}dynamic_RL_model_N{N}_B{B}_G{G}_s{samples}_e{epoch}_b{beta}_lr{lr}_bd{beta_decay}.pth"
...
# Training loop using REINFORCE with model saving
def train_RL_model(model, optimizer, training_data, num_epochs, lr, beta, beta_decay, save_interval=100, save_path=f'N{N}_B{B}_G{G}_{name}/'):
    ...
    for epoch in tqdm(range(start_epoch, num_epochs)):
        ...
        for agents in training_data:
            ...
            boolean_tree = boolean_list_to_tree(m.tolist()[0], len(agents))
            reward = analyzeTree(boolean_tree, agents)

            log_probs_actions = m * torch.log(predicted_probabilities + 1e-8) + (1 - m) * torch.log(1 - predicted_probabilities + 1e-8)
            ...
            for name, param in model.named_parameters():
              if param.grad is not None:  # Gradients might not exist for all params
                  if torch.isnan(param.grad).any():
                      ...
                      logging.info(f"NaN in gradient of {name} during training with agents: {agents}, reward: {reward}, loss: {loss}, log_probs: {log_probs}, current_beta: {current_beta}, entropy: {entropy}, probs: {predicted_probabilities}")
                  if torch.isinf(param.grad).any():
                      ...
                      logging.info(f"Infinity in gradient of {name} during training with agents: {agents}, reward: {reward}, loss: {loss}, log_probs: {log_probs}, current_beta: {current_beta}, entropy: {entropy}, probs: {predicted_probabilities}")
            ...
            for name, param in model.named_parameters():
              if torch.isnan(param).any():
                  ...
                  logging.info(f"NaN in parameter: {name} after optimizer step with agents: {agents}, reward: {reward}, loss: {loss}, log_probs: {log_probs}, current_beta: {current_beta}, entropy: {entropy}, probs: {predicted_probabilities}")
              if torch.isinf(param).any():
                  ...
                  logging.info(f"Infinity in parameter: {name} after optimizer step with agents: {agents}, reward: {reward}, loss: {loss}, log_probs: {log_probs}, current_beta: {current_beta}, entropy: {entropy}, probs: {predicted_probabilities}")
        ...
        if (epoch + 1) % save_interval == 0:
            ...
            model_filename = f"{save_path}dynamic_RL_model_N{N}_B{B}_G{G}_s{len(training_data)}_e{epoch}_b{beta}_lr{lr}_bd{beta_decay}.pth"
            torch.save(model.state_dict(), model_filename)
...
# Save results to CSV
df_results.to_csv(f'RLtuning_N{N}_B{B}_G{G}_{name}.csv', index=False)
